...
for b1=1:length(uu)
    hold on
    plot([b1-.5 b1-.5],[.5 11.5],'k');
    plot([.5 11.5],[b1-.5 b1-.5],'k');
    for b2=b1:length(uu)
...
for s=1:16
    ...
    subjnets=dir(['networks/net_subj' num2str(s) '_neutral' '*.mat']); % the intrinsic are the neutral
...
save(['neutral_networks/instrinsic_nets.mat'],'intrinsic_nets','ids');
...
for c1=1:7
    ...
    for s=1:16
	    ...
	    adj=adj+adj';
	    save(['vsneutral_networks/' nets1(n).name],'adj');
        end
