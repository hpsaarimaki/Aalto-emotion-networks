function regress_neutral(dd,bl)
	...
	for s=1:16
		disp(num2str(s));
    	subjnets=dir([netfolder 'net_subj' num2str(s) '_neutral' '*.mat']);
	...
	mkdir(['vsneutral_' netfolder])
	save(['avg_' netfolder '/neutral_nets.mat'],'intrinsic_nets','ids');
	...
	for c1=1:7
	    ...
	    for s=1:16
	        ...
	        for n=1:length(nets1);
        		...
        		save(['vsneutral_' netfolder nets1(n).name],'adj');
	        end
