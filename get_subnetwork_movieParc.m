%clear all
%close all
function get_subnetwork_movieparc(EN)
...
for r=1:length(rois)
    ...
    if(subnet_ids(r,1)>0);
        subnet_labels{subnet_ids(r,1),1}=strrep(rois(r).groupLabel2,' ','');
    end
end
...
if(0) % obsolete code for storing each in a separete file, not efficient
    for i=1:length(subids)
        disp(['making subnetworks_movieparc/' subnet_labels{subids(i)}]);
        mkdir(['subnetworks_movieparc/' subnet_labels{subids(i)}]);
    end    
    for n=1:length(nets);
        load(['networks/' nets(n).name])
        disp(['loading networks_movieparc/' nets(n).name])
        for id=1:length(subids)
            ...
            outfile=['subnetworks_movieparc/' subnet_labels{thisid} '/' nets(n).name]; % it already has the .mat at the end
...
save(['./' netprefix 'subnetworks_movieparc/all_subnetworks.mat'],'all_subnetworks','class_labels','all_subnet_labels')
